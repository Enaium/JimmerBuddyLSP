 ...
 * @author Enaium
 */
class DocumentSemanticTokensFullService(val documentManager: DocumentManager) : DocumentServiceAdapter() {
        ...
        return CoroutineScope(Dispatchers.Default).future {
            document.token.tokens.forEach { token ->
                when (token.type) {
                    ...
                    DtoLexer.IMPLEMENTS, DtoLexer.AS, DtoLexer.NULL,
                    DtoLexer.OR, DtoLexer.AND, DtoLexer.IS -> {
                        addToken(token, SemanticType.KEYWORD)
                    }
                }
            }

            document.cst.exportStatement()?.also { exportStatement ->
                ...
                addToken(
                    exportStatement.typeParts().qualifiedName().parts.lastOrNull(),
                    SemanticType.TYPE
                )
                exportStatement.PACKAGE()?.also {
                    addToken(it.symbol, SemanticType.KEYWORD)
                }
                exportStatement.packageParts().qualifiedName().parts.forEach { part ->
                    addToken(part, SemanticType.NAMESPACE)
                }
            }
            document.cst.importStatement().forEach { importStatement ->
                importStatement.IMPORT()?.also {
                    addToken(it.symbol, SemanticType.KEYWORD)
                }
                if (importStatement.importedType().isEmpty()) {
                    ...
                    addToken(
                        importStatement.qualifiedName().parts.lastOrNull(),
                        SemanticType.TYPE
                    )
                } else {
                    importStatement.qualifiedName().parts.forEach { part ->
                        addToken(part, SemanticType.NAMESPACE)
                    }
                    importStatement.importedType().forEach { importedType ->
            ...
            document.cst.dtoType().forEach { dtoType ->
                dtoType.modifier().forEach { modifier ->
                    modifier(modifier)
                }
                annotations(dtoType.annotation())
                addToken(dtoType.name, SemanticType.STRUCT)
                dtoType.superInterfaces?.typeRef()?.forEach { superInterface ->
    ...
    private fun annotationArguments(annotationArguments: DtoParser.AnnotationArgumentsContext) {
        annotationArguments.annotationNamedArgument()?.forEach { namedArgument ->
    ...
    private fun annotations(annotations: List<DtoParser.AnnotationContext>) {
    ...
    private fun modifier(modifier: DtoParser.ModifierContext) {
        addToken(modifier.INPUT()?.symbol, SemanticType.KEYWORD)
        addToken(modifier.SPECIFICATION()?.symbol, SemanticType.KEYWORD)
        addToken(modifier.UNSAFE()?.symbol, SemanticType.KEYWORD)
        addToken(modifier.FIXED()?.symbol, SemanticType.KEYWORD)
        addToken(modifier.STATIC()?.symbol, SemanticType.KEYWORD)
        addToken(modifier.DYNAMIC()?.symbol, SemanticType.KEYWORD)
        addToken(modifier.FUZZY()?.symbol, SemanticType.KEYWORD)
    }

    private fun typeRef(typeRef: DtoParser.TypeRefContext) {
        typeRef.QUESTION_MARK()?.also { optional ->
            addToken(optional.symbol, SemanticType.KEYWORD)
        }
        typeRef.qualifiedName().parts.forEach { part ->
            addToken(part, SemanticType.TYPE_PARAMETER)
        }
        typeRef.genericArgument().forEach { genericArgument ->
            genericArgument.typeRef()?.also { typeRef(it) }
        }
        typeRef.genericArgument().firstOrNull()?.also { genericArgument ->
            genericArgument.MULTIPLY()?.also { wildcard ->
                addToken(wildcard.symbol, SemanticType.KEYWORD)
            }
    ...
    private fun macro(macro: DtoParser.MacroContext) {
        ...
        macro.QUESTION_MARK()?.also { optional ->
            addToken(optional.symbol, SemanticType.KEYWORD)
        }
        macro.EXCLAMATION_MARK()?.also { required ->
            addToken(required.symbol, SemanticType.KEYWORD)
        }
    ...
    private fun positiveProp(positiveProp: DtoParser.PositivePropContext) {
        addToken(positiveProp.prop, SemanticType.PROPERTY)
        positiveProp.configuration()?.takeIf { it.isNotEmpty() }?.forEach { configuration ->
            configuration(configuration)
        }
        annotations(positiveProp.annotation())
        positiveProp.modifier()?.also { modifier(it) }
        ...
        positiveProp.func()?.also { func ->
            func.Identifier().forEach {
                addToken(it.symbol, SemanticType.FUNCTION)
            }
            addToken(func.name, SemanticType.KEYWORD)
            addToken(func.insensitive, SemanticType.FUNCTION)
            addToken(func.prefix, SemanticType.FUNCTION)
            addToken(func.suffix, SemanticType.FUNCTION)
        }
        positiveProp.alias()?.also {
            addToken(it.Identifier().symbol, SemanticType.VARIABLE)
        }
        positiveProp.bodySuperInterfaces?.typeRef()?.forEach { superInterface ->
            typeRef(superInterface)
        }
    ...
    private fun propPath(propPath: DtoParser.PropPathContext) {
        propPath.Identifier().forEach { part ->
            addToken(part.symbol, SemanticType.PROPERTY)
        }
    }
    ...
    private fun body(body: DtoParser.DtoBodyContext) {
        ...
        body.explicitProp().forEach { prop ->
            ...
            prop.aliasGroup()?.also { aliasGroup ->
                ...
                aliasGroup.aliasGroupBody()?.also { body ->
                    body.positiveProp()?.takeIf { it.isNotEmpty() }?.forEach { alias ->
                        positiveProp(alias)
                    }
                    body.macro()?.takeIf { it.isNotEmpty() }?.forEach { macro ->
                        macro(macro)
